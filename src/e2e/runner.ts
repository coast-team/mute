...
export class Runner {
  ...
  public async launchRunners() : Promise<void> {
    this.prepareVariablesBeforeRun(gv.scenarioToRun)
    try {
      if (process.argv[3] === 'null') {
        // placing null as the second browser means that we want to run a solo scenario
        await this.tool.launchRunner(gv.pathUser1, gv.browser1)
      } else {
        await Promise.race([this.tool.launchRunner(gv.pathUser1, gv.browser1), this.tool.launchRunner(gv.pathUser2, gv.browser2)])
      }
   ...
   */
  public prepareVariablesBeforeRun(scenarioName: string) : void{
