...
//      - a. The editor is accessible    
    ...
    await t.expect(editorText).eql('access', '2.a. The editor is accessible')
...
//      - g. The second user has added some text to the document
    editorText = await tool.getTextWrittenInTheEditor(t)
    await t.expect(editorText).eql(gv.onlineExpectedTextAccessTest, 'g. The second user has added some text to the document')
