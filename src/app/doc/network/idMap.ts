...
export class IdMap {
   ...
   * @param muteCoreId
   */
  addIds(networkId: number, muteCoreId: number): void {
   ...
   */
  removeIds(muteCoreId: number): void {
  ...
  getMuteCoreId(networkId: number): number {
    return this.networkIdToMuteCoreIdMap.get(networkId)
  }

  getNetworkId(muteCoreId: number): number {
